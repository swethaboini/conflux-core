...
public class SavingsAccountCharge extends AbstractPersistable<Long> {
	...
	public static SavingsAccountCharge createNewFromJson(
			final SavingsAccount savingsAccount, final Charge chargeDefinition,
		...
		MonthDay feeOnMonthDay = command
				.extractMonthDayNamed(feeOnMonthDayParamName);
		Integer feeInterval = command
				.integerValueOfParameterNamed(feeIntervalParamName);
		...
		feeOnMonthDay = (feeOnMonthDay == null) ? chargeDefinition
				.getFeeOnMonthDay() : feeOnMonthDay;
		feeInterval = (feeInterval == null) ? chargeDefinition.getFeeInterval()
				: feeInterval;

		return new SavingsAccountCharge(savingsAccount, chargeDefinition,
				amount, chargeTime, chargeCalculation, dueDate, status,
				feeOnMonthDay, feeInterval);
	}

	public static SavingsAccountCharge createNewWithoutSavingsAccount(
			final Charge chargeDefinition, final BigDecimal amountPayable,
			final ChargeTimeType chargeTime,
			final ChargeCalculationType chargeCalculation,
			final LocalDate dueDate, final boolean status,
			final MonthDay feeOnMonthDay, final Integer feeInterval) {
		return new SavingsAccountCharge(null, chargeDefinition, amountPayable,
				chargeTime, chargeCalculation, dueDate, status, feeOnMonthDay,
				feeInterval);
	}
	...
	private SavingsAccountCharge(final SavingsAccount savingsAccount,
			final Charge chargeDefinition, final BigDecimal amount,
			final ChargeTimeType chargeTime,
			final ChargeCalculationType chargeCalculation,
			final LocalDate dueDate, final boolean status,
		...
		this.chargeTime = (chargeTime == null) ? chargeDefinition
				.getChargeTimeType() : chargeTime.getValue();
		...
		if (isOnSpecifiedDueDate()) {
			if (dueDate == null) {
				...
				throw new SavingsAccountChargeWithoutMandatoryFieldException(
						"savingsaccount.charge", dueAsOfDateParamName,
						defaultUserMessage, chargeDefinition.getId(),
		...
		if (isMonthlyFee() || isWeeklyFee()) {
			this.feeInterval = (feeInterval == null) ? chargeDefinition
	...
	public void undoPayment(final MonetaryCurrency currency,
	...
	public Money waive(final MonetaryCurrency currency) {
		...
		this.amountWaived = amountWaivedToDate.plus(amountOutstanding)
				.getAmount();
		this.amountOutstanding = BigDecimal.ZERO;
		this.waived = true;
	...
	public void undoWaiver(final MonetaryCurrency currency,
			final Money transactionAmount) {
	...
	private BigDecimal calculateAmountOutstanding(
			final MonetaryCurrency currency) {
		return getAmount(currency).minus(getAmountWaived(currency))
				.minus(getAmountPaid(currency)).getAmount();
	}
	...
	public Map<String, Object> update(final JsonCommand command) {
		...
		if (command.isChangeInLocalDateParameterNamed(dueAsOfDateParamName,
				getDueLocalDate())) {
			final String valueAsInput = command
					.stringValueOfParameterNamed(dueAsOfDateParamName);
		...
		if (command.hasParameter(feeOnMonthDayParamName)) {
			...
			final String actualValueEntered = command
					.stringValueOfParameterNamed(feeOnMonthDayParamName);
	...
	private BigDecimal percentageOf(final BigDecimal value,
		...
		if (isGreaterThanZero(value)) {
			...
			final BigDecimal multiplicand = percentage.divide(
					BigDecimal.valueOf(100l), mc);
	...
	public boolean isPaidOrPartiallyPaid(final MonetaryCurrency currency) {
		...
		final Money amountWaivedOrWrittenOff = getAmountWaived(currency).plus(
				getAmountWrittenOff(currency));
	...
	public Money updatePaidAmountBy(final Money incrementBy) {
		...
		if (incrementBy.isGreaterThanOrEqualTo(amountOutstanding)) {
			...
			final Money amountExpected = Money.of(incrementBy.getCurrency(),
					this.amount);
			this.amountOutstanding = amountExpected.minus(amountPaidToDate)
					.getAmount();
		}
	...
	public BigDecimal updateWithdralFeeAmount(final BigDecimal transactionAmount) {
		...
		} else if (ChargeCalculationType.fromInt(this.chargeCalculation)
				.isPercentageOfAmount()) {
			amountPaybale = transactionAmount.multiply(this.percentage).divide(
	...
	private LocalDate calculateNextDueDate(final LocalDate date) {
		LocalDate nextDueLocalDate = null;
		if (isAnnualFee()) {
			nextDueLocalDate = date.withMonthOfYear(this.feeOnMonth).plusYears(
					1);
			nextDueLocalDate = setDayOfMonth(nextDueLocalDate);
	...
	private LocalDate setDayOfMonth(LocalDate nextDueLocalDate) {
		int maxDayOfMonth = nextDueLocalDate.dayOfMonth().withMaximumValue()
				.getDayOfMonth();
		int newDayOfMonth = (this.feeOnDay.intValue() < maxDayOfMonth) ? this.feeOnDay
				.intValue() : maxDayOfMonth;
	...
	public void updateToPreviousDueDate() {
		if (isAnnualFee() || isMonthlyFee() || isWeeklyFee()) {
			LocalDate nextDueLocalDate = new LocalDate(dueDate);
			if (isAnnualFee()) {
				nextDueLocalDate = nextDueLocalDate.withMonthOfYear(
						this.feeOnMonth).minusYears(1);
				nextDueLocalDate = setDayOfMonth(nextDueLocalDate);
			} else if (isMonthlyFee()) {
				nextDueLocalDate = nextDueLocalDate
						.minusMonths(this.feeInterval);
				nextDueLocalDate = setDayOfMonth(nextDueLocalDate);
			} else if (isWeeklyFee()) {
				nextDueLocalDate = nextDueLocalDate
						.minusDays(7 * this.feeInterval);
				nextDueLocalDate = setDayOfWeek(nextDueLocalDate);
			}
	...
	public boolean isChargeIsOverPaid(final LocalDate nextDueDate) {
		final BigDecimal amountPaid = this.amountPaid == null ? BigDecimal.ZERO
				: amountPaid();
