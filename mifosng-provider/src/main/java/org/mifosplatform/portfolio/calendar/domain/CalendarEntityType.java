...
public enum CalendarEntityType {
	...
	LOAN_RECALCULATION_REST_DETAIL(6,
			"calendarEntityType.loan.recalculation.rest.detail"), //
	LOAN_RECALCULATION_COMPOUNDING_DETAIL(7,
			"calendarEntityType.loan.recalculation.compounding.detail"), CHARGES(
			8, "calendarEntityType.charges");
	...
	public static CalendarEntityType fromInt(final int i) {
		final CalendarEntityType entityType = intToEnumMap.get(Integer
				.valueOf(i));
		return entityType;
	}
	...
	public static boolean isGroup(final String name) {
		return CalendarEntityType.GROUPS.name().equalsIgnoreCase(name) ? true
				: false;
	}
	...
	public static boolean isCenter(final String name) {
		return CalendarEntityType.CENTERS.name().equalsIgnoreCase(name) ? true
				: false;
	}
	...
	public static boolean isLoan(final String name) {
		return CalendarEntityType.LOANS.name().equalsIgnoreCase(name) ? true
				: false;
	}
	...
	public static boolean isClient(final String name) {
		return CalendarEntityType.CLIENTS.name().equalsIgnoreCase(name) ? true
				: false;
	}
	...
	static {
		for (final CalendarEntityType entityType : CalendarEntityType.values()) {
			entityNameToEnumMap
					.put(entityType.name().toLowerCase(), entityType);
		}
	}
	...
	public static boolean isSavings(final String name) {
		return CalendarEntityType.SAVINGS.name().equalsIgnoreCase(name) ? true
				: false;
	}
